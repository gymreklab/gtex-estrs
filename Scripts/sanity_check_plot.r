...
#system("vcftools --site-mean-depth --gzvcf /storage/szfeupe/Runs/GTEx_estr/str_chr1.vcf.gz --out Depth")
#system("vcftools --site-depth --gzvcf /storage/szfeupe/Runs/GTEx_estr/str_chr1.vcf.gz --out Depth")

## Get distribution of step size for out-of-frame changes and repeat number
#system("awk '{print $4"\t"$10"\t"$5"\t"$6}' /storage/szfeupe/Runs/GTEx_estr/stutter_chr1.txt > step_by_repeat")
...
##Summing IUP and IDOWN & plot
stut_conf = step_dist$IUP + step_dist$IDOWN
barplot(stut_conf, names.arg=step_dist$Repeats, ylim=c(0,0.25), xlab="Size of repeat units", ylab="IUP")

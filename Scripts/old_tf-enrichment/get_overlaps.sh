...
cat /storage/szfeupe/Runs/650GTEx_estr/Analysis_by_Tissue/${TISSUE}/PQValues | \
    awk '($8<0.1)' | awk '{print $2 "\t" $4 "\t" $4+1}' > ${TMPDIR}/${TISSUE}_${PREFIX}_eSTRs.bed
...
giggle search -q ${TMPDIR}/${TISSUE}_${PREFIX}_eSTRs.bed.gz -i ${INDEXDIR} | \
    ...
    cut -f 1,3 | sed 's/overlaps://' | sort > ${TMPDIR}/${TISSUE}_${PREFIX}_eSTR_overlap.tab
...
join -t $'\t' ${TMPDIR}/${TISSUE}_${PREFIX}_eSTR_overlap.tab ${TMPDIR}/${TISSUE}_${PREFIX}_allSTR_overlap.tab | \
    ...
    ${TMPDIR}/${TISSUE}_${PREFIX}_table.tab

# Perform Fisher exact test
./fisher_exact.py ${TMPDIR}/${TISSUE}_${PREFIX}_table.tab > ${OUTDIR}/${TISSUE}_${PREFIX}_enrich.tab

...
def main():
    ...
    sys.stdout.write("\t".join(map(str, ["CHROM", "START","END","hwe_p", "het","mean_allele", "obs_het", "obs_hom"]))+"\n")
    
    if args.unrelated_samples:
        samples = [item.strip() for item in open(args.unrelated_samples, "r").readlines()]
    else: samples = reader.samples    #[]
    for record in reader:
        hwe_p, het, mean_allele,obs_het,obs_hom = GetLocusStats(record, samples=samples)
        #num_calls = record.INFO["AN"]/2
        sys.stdout.write("\t".join(map(str, [record.CHROM, record.INFO["START"], record.INFO["END"], \
                                                 hwe_p, het, mean_allele,obs_het,obs_hom]))+"\n") #num_calls,
        sys.stdout.flush()

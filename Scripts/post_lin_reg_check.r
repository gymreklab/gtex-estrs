...
#remove = read.table("homopoly", sep="\t", header=FALSE)
...
##perform the FDR adjustment of pvalues
SS = linout$p.wald
ad_pval = p.adjust(SS, method="fdr")
linout$qvalue <- ad_pval

## save data into a file
write.table(linout, file="Adjusted_Lin_Reg_Out", sep="\t")

## Get the most significant after FDR correction
estr = subset(linout, qvalue <=0.05)

#Save it
write.table(estr, file="possible_estrs", sep="\t" )
